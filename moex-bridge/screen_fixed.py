...
def main():
    ...
    stats={"rows":0,"face":0,"offer":0,"last":0,"purchase_price":0,"coupon_pct":0,"maturity":0,"in_horizon":0,"calculable":0}
    samples=[]
    for raw in INPUT.read_text(encoding="utf-8").splitlines():
        if not raw.strip(): continue
        stats["rows"]+=1
        r=json.loads(raw)
        face=f(r.get("face")); offer=f(r.get("offer")); last=f(r.get("last")); price=f(r.get("purchase_price_pct")); coupon=f(r.get("coupon_pct")); nkd=f(r.get("nkd")) or 0.0
        ...
        if price is not None: stats["purchase_price"]+=1
        ...
        if MIN_DAYS <= days <= MAX_DAYS:
            stats["in_horizon"]+=1
            if len(samples)<20:
                samples.append({"secid":r.get("secid"),"name":r.get("name"),"offer":offer,"last":last,"price":price,"coupon_pct":coupon,"maturity":r.get("maturity"),"days":days})
        if face is None or price is None or coupon is None: continue
        if not (MIN_DAYS <= days <= MAX_DAYS): continue
        stats["calculable"]+=1
        clean=face*price/100.0; dirty=clean+nkd
        ...
        candidates.append({"secid":r.get("secid"),"isin":r.get("isin"),"shortname":r.get("shortname"),"name":r.get("name"),"board":r.get("board"),"offer_pct":offer,"last_pct":last,"screen_price_pct":price,"nkd_rub":nkd,"dirty_rub":round(dirty,4),"coupon_pct":coupon,"maturity":r.get("maturity"),"days":days,"approx_simple_pct":round(simple,4),"moex_yield_pct":f(r.get("moex_yield")),"issue_size":r.get("issue_size"),"value_today":r.get("value_today"),"num_trades":r.get("num_trades"),"offer_date":r.get("offer_date"),"buyback_price":r.get("buyback_price"),"list_level":r.get("list_level")})
    candidates.sort(key=lambda x:(-x["approx_simple_pct"],x["days"],x["secid"] or ""))
    payload={"generated_from":"moex-bridge/data/market.jsonl","criteria":{"days_to_maturity_min":MIN_DAYS,"days_to_maturity_max":MAX_DAYS,"approx_simple_yield_min_pct":MIN_APPROX_SIMPLE,"price":"OFFER if available, else LAST, else previous close","note":"Pre-screen only. Verify fixed/floater, ratings, amortizations and exact simple yield."},"stats":stats,"horizon_samples":samples,"count":len(candidates),"candidates":candidates}
